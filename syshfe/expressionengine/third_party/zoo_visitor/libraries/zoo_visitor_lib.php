...
class Zoo_visitor_lib
{
	...
	function login_form($ret, $error_handling)
	{
		$vars = array();
		$vars[0] = array('email' => '',
						'error:email' => '',
		...
		if(isset($_POST['visitor_action']) && $_POST['visitor_action'] == 'login')
		{
			...
			else
			{
				...
				{
					$this->EE->output->show_user_error(FALSE, $result[1]);
				}
				else
				{
					$vars[0] = array('email' => $_POST['email'],
									'error:email' => implode('<br/>', $result[1] ));
				}
			}
	...
	function forgot_password($tagdata, $ret, $error_handling, $is_ajax_request = 'no'){
		...
		$data['hidden_fields']	= array(
										...
										'RET'	=> $ret,
										'FROM'	=> ($MA->in_forum == TRUE) ? 'forum' : ''
									  );

		if ($MA->in_forum === TRUE)
		{
			$data['hidden_fields']['board_id'] = $MA->board_id;
		}
	...
	function update($do_update = TRUE)
	{
		...
		if(array_key_exists('password', $_POST) || array_key_exists('username', $_POST) || array_key_exists('screen_name', $_POST)){
			...
			// 			{
			// 				
			// 				$errors = array("password" => array($this->EE->lang->line('confirm_password')));
			// 				
			// 			}
		...
		if(array_key_exists('email', $_POST) && $_POST['email'] != $this->EE->session->userdata['email']){
			...
			$errors = array_merge($errors, $this->update_email($do_update));	
		}
	...
	function get_override_screen_name(){
	    ...
	    if ($query->num_rows() > 0)
		{
			...
			foreach ($query->result_array() as $row)
			{
				$value = (isset($_POST[$row['field_name']])) ? $_POST[$row['field_name']] : ''; 
				$screen_name = str_replace('field_id_'.$row['field_id'], $value, $screen_name);
	...
	// =========================
	function update_email($do_update = TRUE)
	{
	...
	function register_member($ext, $doRegister = TRUE, $error_handling = '')
	{
		...
		*/
		...
		if (isset($selected_group_id) && $this->EE->config->item('req_mbr_activation') != 'email' && $this->EE->config->item('req_mbr_activation') != 'manual')
		{
	...
	function update_entry_title($entry_id){
		...
		if($member != FALSE)
		{
			...
			if($titlecheck != '')
			{
				...
				if($url_title != FALSE)
				{
					$this->EE->db->update('channel_titles', array('title' => $title, 'url_title' => $url_title), 'entry_id = '.$entry_id);
				}
			}
		}
