...
class Datagrab_model extends CI_Model {
	...
	function do_import( $datatype, $settings ) {
		...
		while( $item = $datatype->next() ) {
			...
			if( isset( $this->settings["config"][ "url_title" ] ) ) {
				...
				// Add prefix
				if( $this->channel_defaults["url_title_prefix"] != "" && $url_title != "" ) {
			...
			if( isset( $this->settings["cf"]["ajw_solspace_tag"] ) ) {
				$tags = explode(",", $datatype->get_item( $item, $this->settings["cf"]["ajw_solspace_tag"] ));
 ...
 // print_r( $data );
	...
	*/
	...
	function _setup_categories( $item, $entry_id = FALSE ) {
		...
		foreach( explode("|", $c_groups) as $cat_group_id ) {
			...
			$new = $this->_find_and_create_categories( 
				isset( $this->settings["config"][ "category_value_".$cat_group_id ] ) ? $this->settings["config"][ "category_value_".$cat_group_id ] : '',
				$cat_field,
				$cat_delimiter,
				$cat_group_id,
	 ...
	 *
	 * @param string $category_value Contains default category name or id
	...
	function _prepare_update_data( $entry_id, &$data ) {
		...
		*/
		...
		//print_r( $data ); print_r( $this->settings ); exit;
		
		$this->db->select( "title, entry_date, status, author_id" );
		$this->db->where( "entry_id", $entry_id );
		$query = $this->db->get( "exp_channel_titles" );
		$row = $query->row_array();
